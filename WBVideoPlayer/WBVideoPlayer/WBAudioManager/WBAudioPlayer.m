...
static void sessionPropertyListener(void *                  inClientData,
                                    ...
                                    const void *            inData)
{
    WBAudioPlayer *sm = (__bridge WBAudioPlayer *)inClientData;
...
{
    WBAudioPlayer *sm = (__bridge WBAudioPlayer *)inRefCon;
...
- (BOOL) checkAudioRoute
{
    ...
    WBVPLog(@"AudioRoute: %@", _audioRoute);
    return YES;
}
...
- (BOOL) setupAudio
{
    ...
    WBVPLog(@"Current output bytes per sample: %ld", _numBytesPerSample);
    WBVPLog(@"Current output num channels: %ld", _numOutputChannels);
...
- (BOOL) checkSessionProperties
{
    ...
    WBVPLog(@"We've got %lu output channels", newNumChannels);
    ...
    WBVPLog(@"Current sampling rate: %f", _samplingRate);
    ...
    WBVPLog(@"Current output volume: %f", _outputVolume);
               ...
               ioData: (AudioBufferList *) ioData
{
    ...
    if (_playing && _outputBlock ) {
        ...
        else if (_numBytesPerSample == 2) // then we need to convert SInt16 -> Float (and also scale)
        {
            ...
            WBVPLog(@"Buffer %u - Output Channels %u - Samples %u",
